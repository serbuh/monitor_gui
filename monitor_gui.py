...
class Window():
    def __init__(self, master, conn):
        ...
        self.master.after(10, self.add_labels)
    ...
    def add_labels(self):
        ...
        for row_count, field_name in enumerate(status_dict.keys()):
            ...
            # Label
            self.string_var_dict[field_name] = tk.StringVar()             # Create new StringVar
            self.string_var_dict[field_name].set(status_dict[field_name]) # Set text to the StringVar
            tk.Label(master = frm_msg, width=max_name_len, relief=tk.RAISED, bd=2, text = field_name).grid(row=0, column=0)
            tk.Label(master = frm_msg, width=max_value_len, relief=tk.RAISED, bd=2, textvariable = self.string_var_dict[field_name]).grid(row=0, column=1) # bind to StringVar
